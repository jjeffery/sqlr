...
type Stmt struct {
	...
	output    struct {
		mutex  sync.RWMutex
		colmap map[string][]*column.Info // map sql query to output column info
	}
	autoIncrColumn *column.Info
}

// Prepare a statement given the row and the sql. If an existing
// statement exists in the cache it will be reused.
...
// inferRowType returns the expected type for rows. This type is expected to be a struct.
func inferRowType(row interface{}) (reflect.Type, error) {
	...
	if rowType.Kind() != reflect.Struct {
		return nil, errors.New("expected row type to be a struct")
	}
	return rowType, nil
}

func newStmt(rowType reflect.Type, sql string) (*Stmt, error) {
	stmt := &Stmt{}
	// no check that rowType is a struct: this has been checked in inferRowType
	...
	// Because this statement can handle different dialects and column naming conventions,
	// the output column names may vary. For this reason the output columns are keyed on
	// the SQL query string used.
	stmt.output.colmap = make(map[string][]*column.Info)
...
func (stmt *Stmt) Exec(db Execer, dialect Dialect, columnNamer ColumnNamer, row interface{}, args ...interface{}) (int, error) {
	...
	expandedQuery, expandedArgs, err := wherein.Expand(stmt.query.SQLString(dialect, columnNamer), args)
	if err != nil {
		return 0, err
	}
...
func (stmt *Stmt) Select(db Queryer, dialect Dialect, columnNamer ColumnNamer, rows interface{}, args ...interface{}) (int, error) {
	...
	if destType == stmt.rowType {
		...
		return stmt.selectOne(db, dialect, columnNamer, rows, destValue, args)
	}
	...
	query := stmt.query.SQLString(dialect, columnNamer)
	...
	// No need to use the expanded query as the query for getting outputs: the expanded
	// query does not alter the columns that are returned by the query.
	outputs, err := stmt.getOutputs(sqlRows, query, columnNamer)
	if err != nil {
		return 0, err
	}
...
func (stmt *Stmt) selectOne(db Queryer, dialect Dialect, columnNamer ColumnNamer, dest interface{}, rowValue reflect.Value, args []interface{}) (int, error) {
	query := stmt.query.SQLString(dialect, columnNamer)
	expandedQuery, expandedArgs, err := wherein.Expand(query, args)
	if err != nil {
		return 0, err
	...
	// No need to use the expanded query as the query for getting outputs: the expanded
	// query does not alter the columns that are returned by the query.
	outputs, err := stmt.getOutputs(rows, query, columnNamer)
	if err != nil {
		return 0, err
	}
...
// getOutputs works out the order of the columns that have been returned by the query.
// It does this by comparing the column names associated with the structure fields and
// the column names returned by the query. Because this is a reasonably expensive operation,
// the results are cached. The key to the cache is the SQL query text, which varies with
// the dialect and naming convention used. Most of the time, when there is only one dialect
// and one naming convention in use, the cache for each statement will only have one entry.
func (stmt *Stmt) getOutputs(rows *sql.Rows, query string, columnNamer ColumnNamer) ([]*column.Info, error) {
	stmt.output.mutex.RLock()
	outputs := stmt.output.colmap[query]
	...
	if outputs = stmt.output.colmap[query]; outputs != nil {
		return outputs, nil
	}

	columnMap := make(map[string]*column.Info)
	for _, col := range stmt.columns {
		columnName := columnNameFromInfo(col, columnNamer)
	...
	if columnNotFound {
		...
		for i, columnName := range columnNames {
			...
			delete(columnMap, columnNameFromInfo(col, columnNamer))
		}
	...
	stmt.output.colmap[query] = outputs
	return outputs, nil
}
...
func (stmt *Stmt) scanSQL(query string) error {
	...
	for scan.Scan() {
		...
		case scanner.PLACEHOLDER:
			...
			buf.WritePlaceholder(counter.Next())
	...
	if stmt.query, err = buf.Finish(); err != nil {
		return err
	}
	return nil
}
